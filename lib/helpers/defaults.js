  ...
  const defaults = {
    ...
    features: {
      ...
      requestObjects: {
        ...
        /*
         * features.requestObjects.requireEncryptedRequestObject
         *
         * description: Makes the use of encrypted request objects required on a per-request basis
         * to allow an OP policy to dictate when request object encryption must be used. This helper
         * is only called when the encryption feature is enabled and the client provided an unencrypted
         * request object. It is meant to fill a gap in available client metadata definitions that
         * currently don't allow to specify a client must use encryption.
         *
         * recommendation: Use `return true` when it's required to provide an encrypted Request Object.
         * recommendation: Use `return false` (default) when use of encrypted Request Objects is optional.
         */
