...
module.exports = async function processRequestObject(PARAM_LIST, rejectDupesMiddleware, ctx, next) {
  ...
  if (features.encryption.enabled) {
    const { length } = params.request.split('.');
    switch (true) {
      case length === 5: {
      ...
      case length === 3 && !!features.requestObjects.requireEncryptedRequestObject(ctx, client): {
        throw new InvalidRequestObject('Request Object Encryption is required by Authorization Server policy');
      }
